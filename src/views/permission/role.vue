...
const handleAssigned = (row: any) => {
  ...
  getMeunRole(row.ID).then((data: any) => {  // 改为 row.ID
      ...
      dataText.forEach((item: any) => {
        if (item.childMenu !== null) {
          item.childMenu.forEach((i: any) => {
            if (i.childMenu == null) {
              roleData.value.push(i.ID);  // 改为 i.ID
            } else {
              i.childMenu.forEach((v: any) => {
                if (v.childMenu !== null) {
                  v.childMenu.forEach((c: any) => {
                    roleData.value.push(c.ID);  // 改为 c.ID
                  })
                } else {
                  roleData.value.push(v.ID)  // 改为 v.ID
                }
              })
            }
          });
        } else {
          roleData.value.push(item.ID);  // 改为 item.ID
        }
      });
    }
...
const handleDelete = (row: any) => {
  ElMessageBox.confirm("确定删除", "确认操作", {
    ...
    .then(() => {
      deleteRole(row.ID).then((data: any) => {  // 改为 row.ID
        if ((data.code = 100200)) {
          getData();
          ElNotification({
            title: "提示信息",
